...
#NEU 0.98b: auch Hexbutton einheitlich, wegen custom colors
CustomColours = [wx.NullColour] * 16	#NEU 0.98: die Custom colours links unten merken, anfangs undefiniert
def MyColourDialog(win, colour):
  dlg = wx.ColourDialog(wx.GetTopLevelParent(win))
  
  data = dlg.GetColourData()
  data.SetChooseFull(True)		#voller Dialog, wie bei HexColourButton implizit auch
  data.SetColour(colour)		#Init mit Farbe aus der Zelle
  
  #NEU 0.98: die Custom colours links REIN
  global CustomColours
  map(data.SetCustomColour, range(16), CustomColours)
  
  #NEUE FARBE LAUT DIALOG [OK] ODER None [CANCEL]
  if dlg.ShowModal() == wx.ID_OK:	colour = data.GetColour()
  else:					colour = None
  
  #NEU 0.98: die Custom colours links RAUS (egal ob OK oder Cancel)
  CustomColours = map(data.GetCustomColour, range(16))
...
class LedGrid (wx.grid.Grid):
  ...
  def MultiCells(self, pivotcell):
    "NEU 0.98: mehrfache Selektion aus 3 verschiedenen Quellen sauber sammeln"
    
    #1.fuer 1 oder mehrere selectete Bloecke erhaelt man nur die Ecken
    multicells = []
    for (x0,y0),(x1,y1) in zip(self.GetSelectionBlockTopLeft(),self.GetSelectionBlockBottomRight()):
      multicells += sum(map(lambda x: map(lambda y: (x,y), range(y0,y1+1)), range(x0,x1+1)),[])
    
    #2.Einzel-Zellen erhaelt man direkt
    multicells += self.GetSelectedCells()
    
    #3.wenn nur 1 Zelle den Kursor hat ("ohne Selektion"), dann habe ich nur mein eigenes firstselect
    if multicells and self.firstselect not in multicells:
      multicells.append(self.firstselect)
    
    #4.die genannte Pivot-Zelle [aus dem Event] definiert vorne ran
    try: multicells.remove(pivotcell)	#rausnehmen falls es drin ist
    except ValueError: pass
    if pivotcell:
      multicells.insert(0, pivotcell)	#und dann definiert vorne ran
  ...
  def OnColorPicker(self, event):
    ...
    #NEU 0.98: Multi-Selektion !
    #die aktuelle Zelle: Doppelklick ist ein Grid-Event, die Zelldaten sind einfach
    if isinstance(event, wx.grid.GridEvent):
      celllist = self.MultiCells((event.GetRow(), event.GetCol()))
    
    #fuer Hotkey/Menu muss ich aktuelle selber suchen
    else:
      celllist = self.MultiCells(self.firstselect)
    
    #INPUT: die Zellen analysieren
    hexlist = map(lambda (r,c):self.GetCellValue(r,c),celllist)	#Liste aller hex-Farben
    badlist = filter(lambda hex: hex[0]=='(', hexlist)		#gefiltert: nur die boesen/virtuellen
    
    if not hexlist or badlist:		#keine Selektion oder virtuelle Farben dabei
      event.Skip()
      return
    
    hex = hexlist[0]			#sonst Farbe der ersten/einzigen Zelle als Vorgabe
    
    colour = MyColourDialog(self, hex2col( hex ))
    if colour != None:			#neue Farbe in die Zelle[n]
      #OUTPUT: die Zellen aendern
      #NEU 0.98: Multi-Selektion !
      if len(celllist)==1:		#single cell, 1 step undo wie bisher
        self.undolast = celllist[0] + (hex,)
      else:				#multi: volles Undo
        self.undolast = self.GetAllCells()
      
      hex = col2hex( colour )	#neue Farbe fuer alle
      for (row,col) in celllist:
        self.SetLEDCell(row,col, hex)
  ...
  def OnCellRightClick(self, event):
    ...
    #die aktuelle Zelle (bad ist egal (##CLEAN: eigentlich ueberall), virtuelle Farbe)
    row,col,hex,_ = self.EventCelldata(event)
    
    if event.ControlDown():		#mit Ctrl: "PICK" in den Button rueber
      self.ClearSelection()		#nur aus einer Zelle
      ColourButton.SetHex( hex )
    
    else:				#ohne: "FILL" die Zelle[n] faerben
      #NEU 0.98: Multi-Selektion !
      celllist = self.MultiCells((row,col))
      if celllist:
        if len(celllist)==1:		#single cell, 1 step undo wie bisher
          self.undolast = celllist[0] + (self.GetCellValue(*celllist[0]),)
        else:				#multi: volles Undo
          self.undolast = self.GetAllCells()
        
        hex = ColourButton.GetHex()	#neue Farbe fuer alle
        for (row,col) in celllist:
          self.SetLEDCell(row,col, hex)
    
    self.ClearSelection()		#Right-Click-Drag macht Aerger mit Selektion
    event.Skip()
...
class HexColourButton (wx.lib.colourselect.ColourSelect):
  ...
  #NEU 0.98b: OnClick von Vater ColourSelect kopiert und ueberschrieben,
  #  um auch hier die custom colours behandeln
  def OnClick(self, event):
    if MainMode=='M': return		#META: kein Farbdialog
      ##LATER: wie in Gridzelle die Liste der benutzten/unbenutzten ?
    
    colour = MyColourDialog(self, self.colour)
    if colour != None:			#neue Farbe uebernehmen
...
class MyFrame (wx.Frame):
  ...
  def SizeDialog(self):
    "NEU 0.98: Helper fuer Sizedialog, jetzt mit MAX-Begrenzung"
    #Sizedialog fuer 2 Werte
    size = UseTableDialog(self, MainProg.size, ["Width","* Height"], [int0,int0])
    
    if not size: return size		#Dialog [Cancel] durchreichen
    if size[0]*size[1] > MAXKACHEL:
      LogMessageDialog(MSG_ERROR, "%d Kacheln sind nicht addressierbar, Maximum ist 127" % (size[0]*size[1]))
      return []				#wegen MAX abgelehnt
