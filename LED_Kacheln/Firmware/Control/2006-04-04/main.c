...
const char __attribute__ ((space(auto_psv))) StartScreen[4][17] = {"  Kachel-Ctrl  ", "  Version 2.01  ", "   02.03.06     ","               "};
//nst char __attribute__ ((space(auto_psv))) StartScreen[4][17] = {"  Kachel-Ctrl  ", "  TEST UPLOAD ", "   02.03.06     ","               "};
...
// Hex-Schalter Codes
	...
	#define GATEWAY			7
	#define PLAYFILE		3
	#define DOWNLOADFILE	1
	#define FUNKSTEUERN		2
	#define IRSTEUERN		6	
	#define FBLERNEN		0
	#define FIRMWAREUPDATE	8
#endif
...
void SendKachel(int adr, int rot, int gruen, int blau);
int CountZeros(int );
...
int itemp;
	...
	case 2:				// Alle kacheln durchfahren
		...
		SendKachel(KachelList[AktKachel], PlaySzeneAkt.Kachel[AktKachel][ROT],
										...
										PlaySzeneAkt.Kachel[AktKachel][BLAU]);
		AktKachel++;
	...
	case 3:
		...
		for (k = 0; k < 3; k++)
			AutoSzeneTemp.Farbe[k] = CalcFarbe( PlaySzeneAkt.Kachel[AktKachel][k], 
												PlaySzeneNext.Kachel[AktKachel][k]);

		SendKachel(KachelList[AktKachel], AutoSzeneTemp.Farbe[ROT],
										AutoSzeneTemp.Farbe[GRUEN],
										AutoSzeneTemp.Farbe[BLAU]);
		AktKachel++;
		if (AktKachel >= AnzKacheln)
			AktKachel = 0;
...
{
	return (long) ( ((float)zeit) * 1000.0 * (float)TempoAkt*0.01 );
}
//***************************************************
		...
		if (Rx1WrPtr != Rx1RdPtr)
		{
			...
			else
			{
				...
				else
				{
					...
					if (RxByte & 0x4)
						RxBuff[3] |= 0x80;
					SendKachel(RxBuff[0], RxBuff[1], RxBuff[2], RxBuff[3]);
					RxBuffCnt = 0;
					HexStatus = 1;
				}
			}
...
//			OwnItoa(&temp[12],TempoAkt,3,NO_NULL);
			SendKachel(0xff, FarbenAkt[0], FarbenAkt[1], FarbenAkt[2]);
			DisplayWriteLine(1,0,temp);
		}
		MenuStateMachine();
		break;
...
#define AD_CHANNELS 0B1111	// Bitmuster, für jeden benutzten 
	...
	ADCSSL = 0; 				// A/D Input Scan Selection Register		
...
void InitUart2(void)
{
	...
	U2BRG = 51;					// = 19.200 Bauf bei 16MHz CPU-Clock (fosc = 64MHz)
...
int MaxFileSize;
	...
	case 'I':		// Version zurückschicken
		...
		MaxFileSize = FLASH_BASEADR - 0x200;		// Max. verfügbarer Speicherplatz
